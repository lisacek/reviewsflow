...
export default function ApiDocs() {
  ...
  const maskedToken = useMemo(() => {
    if (!token) return 'Log in to view token';
  ...
  const bearerHeader = token ? `Authorization: Bearer ${token}` : 'Authorization: Bearer <your_token>';
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="space-y-0">
            ...
            <EndpointSection
                ...
                method="POST"
                path="/instances"
                description="Initialize a new review monitoring instance."
                icon={Play}
                bearer={bearerHeader}
                snippets={{
...
response = requests.get(
    "${API_BASE}/api/reviews/123",
    headers={"Authorization": "${token || 'YOUR_TOKEN'}"}
)`,
                  javascript: `fetch('${API_BASE}/api/reviews/123', {
  headers: { 'Authorization': '${token || 'YOUR_TOKEN'}' }
})`
                }}
            />
  ...
  -H "${bearerHeader}"`,
                  python: `requests.get("${API_BASE}/instances", headers=auth)`,
                  javascript: `fetch('${API_BASE}/instances', { headers: auth })`
                }}
            />
