...
// ... existing variants (containerVariants, itemVariants) ...
    ...
    return (
        <div className="min-h-screen bg-black text-zinc-100 font-sans selection:bg-blue-500/30 relative overflow-hidden">
            ...
            <section className="relative pt-24 pb-32 px-6">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[500px] bg-blue-600/15 rounded-full blur-[120px] -z-10 pointer-events-none" />
                ...
                >
                    <motion.div variants={itemVariants}>
                        <div className="inline-flex items-center px-3 py-1 rounded-full border border-zinc-800 bg-zinc-900/50 text-xs font-medium text-blue-400 mb-6 backdrop-blur-sm">
                    ...
                    <motion.h1 variants={itemVariants} className="mb-6 text-5xl md:text-7xl font-bold tracking-tight leading-tight">
                        <span className="block pb-2">Google Reviews,</span>
                        <span className="block text-white leading-[1.15]">Without the Monthly Fees.</span>
                    </motion.h1>
                    ...
                    <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-center justify-center gap-4">
                        <button onClick={onRegister} className="group h-12 px-8 rounded-full bg-blue-600 hover:bg-blue-500 text-white font-semibold transition-all shadow-[0_0_30px_-10px_rgba(37,99,235,0.6)] hover:shadow-[0_0_40px_-5px_rgba(37,99,235,0.8)] flex items-center gap-2">
                            Start Building <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform"/>
                        </button>
                        {!authed && <button onClick={onLogin} className="h-12 px-8 rounded-full border border-zinc-800 bg-zinc-900/50 hover:bg-zinc-800 hover:text-white text-zinc-200 font-medium transition-all backdrop-blur-sm">
                            Log In
                        </button>}
                    </motion.div>
                </motion.div>
            ...
            <section className="py-32 px-6 relative">
                {/* ... (Same features code) ... */}
                <motion.div animate={{ scale: [1, 1.1, 1], opacity: [0.2, 0.3, 0.2] }} transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }} className="absolute top-1/4 right-0 translate-x-1/3 w-[600px] h-[600px] bg-purple-600/10 rounded-full blur-[120px] -z-10 pointer-events-none" />
                <div className="max-w-6xl mx-auto">
                    ...
                    <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <FeatureCard icon={ShieldCheck} title="Domain Locking" description="Your API key is never exposed. Requests are proxied and only allowed from domains you explicitly whitelist." />
                        <FeatureCard icon={Zap} title="Aggressive Caching" description="We cache Google API responses to minimize quota usage and ensure your site loads instantly, even if Google is slow." />
                        <FeatureCard icon={Layout} title="Headless or UI" description="Use our pre-built, beautiful React widget, or fetch the raw JSON data to build your own custom UI." />
                        <FeatureCard icon={Globe} title="Multi-Locale" description="Serve reviews in multiple languages. Our widget automatically adapts to the user's browser locale settings." />
                        <FeatureCard icon={Code2} title="Zero Dependencies" description="The generated widget script is standalone. No React or heavy libraries required on the client side." />
                        <FeatureCard icon={Star} title="Smart Filtering" description="Only show your best side. Automatically filter out reviews below 4 stars or reviews without text." />
                    </motion.div>
                </div>
            </section>
            ...
            <section className="py-32 px-6 border-t border-zinc-900/50 bg-zinc-950/30 relative overflow-hidden">
                ...
                <div className="max-w-4xl mx-auto">
                    <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-4xl font-bold mb-16 text-center bg-gradient-to-b from-white to-zinc-400 bg-clip-text text-transparent">Deployment Workflow</motion.h2>
                    <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} className="space-y-6">
                        <Step num="01" title="Create an Instance" desc="Paste your Google Maps URL into the Builder. Configure filters, sorting, and caching rules." />
                        <Step num="02" title="Whitelist Domain" desc="Add your production domain (e.g., myshop.com) to the security settings to prevent API theft." />
                        <Step num="03" title="Embed & Forget" desc="Copy the generated HTML snippet. The widget will auto-refresh reviews in the background." />
                    </motion.div>
                </div>
            </section>
...
const BackgroundGrid = () => (
    <div className="absolute inset-0 -z-20 pointer-events-none">
        <svg className="absolute h-full w-full stroke-white/5 [mask-image:radial-gradient(100%_100%_at_top_center,white,transparent)]" aria-hidden="true">
            <defs>
                <pattern id="grid-pattern" width={100} height={100} x="50%" y={-1} patternUnits="userSpaceOnUse"><path d="M.5 200V.5H200" fill="none" /></pattern>
            </defs>
        ...
        >
            <div className="flex items-center justify-between px-4 py-2 border-b border-zinc-800 bg-zinc-900/30">
                <div className="flex gap-2">
                    <button onClick={() => setOs('linux')} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${os === 'linux' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>Linux / macOS</button>
                    <button onClick={() => setOs('windows')} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${os === 'windows' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>Windows (PowerShell)</button>
                </div>
...
const FeatureCard = ({icon: Icon, title, description}) => (
    <motion.div variants={itemVariants} className="group relative p-1 rounded-2xl bg-gradient-to-b from-zinc-800/50 to-zinc-900/50 border border-zinc-800/50 hover:border-zinc-700/80 transition-all duration-300 overflow-hidden">
        ...
        <div className="relative p-6 h-full rounded-xl bg-black/40 backdrop-blur-sm">
            <div className="w-12 h-12 rounded-xl bg-zinc-900/80 border border-zinc-800 flex items-center justify-center mb-5 text-blue-400 group-hover:text-blue-300 group-hover:scale-110 transition-all duration-300 shadow-inner"><Icon className="w-6 h-6"/></div>
...
const Step = ({num, title, desc}) => (
    <motion.div variants={itemVariants} className="flex items-start gap-6 p-6 rounded-2xl border border-zinc-800/50 bg-zinc-900/20 hover:bg-zinc-900/40 hover:border-zinc-700 transition-all duration-300 backdrop-blur-sm relative overflow-hidden group">
        ...
        <div className="relative z-10"><h3 className="text-xl font-bold text-zinc-200 mb-2 flex items-center gap-2">{title} <CheckCircle2 className="w-5 h-5 text-blue-500 fill-blue-500/10"/></h3><p className="text-zinc-400 leading-relaxed">{desc}</p></div>
    </motion.div>
);